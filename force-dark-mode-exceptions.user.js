...
(function () {
    ...
    const tldOffset = (parts.length > 2 && ["co", "com", "org", "net"].includes(parts[parts.length - 2])) ? 3 : 2;
    const root = parts.slice(-tldOffset).join(".");
    ...
    if (isException) {
        const lightModeSheet = new CSSStyleSheet();
        lightModeSheet.replaceSync(`:root { color-scheme: light only !important; }`);

        const applyStyle = (target) => {
            try {
                if (target && !target.adoptedStyleSheets.includes(lightModeSheet)) {
                    target.adoptedStyleSheets = [...target.adoptedStyleSheets, lightModeSheet];
                }
            } catch (err) {}
        };
            ...
            for (const m of ms) {
                for (const n of m.addedNodes) {
                    if (n.nodeType === 1 && n.tagName === 'IFRAME') {
                        n.addEventListener('load', () => applyStyle(n.contentDocument));
                    }
                }
            }
